...
export interface BubbleSliderProps extends SliderOptions {
  ...
  /** Compact 32px hit area for toolbars. */
  size?: "default" | "sm";
  className?: string;
}
...
export function BubbleSlider({
  ...
  const lean = useSpring(
    useTransform(velocity, [-FULL_TILT, 0, FULL_TILT], [1, 0, -1], {
      clamp: true,
    }),
    SPRING_TILT,
  );
  ...
  return (
    <div
      className={cn("mr-video-editor-theme",
        ...
        size === "sm" && "h-8 items-center px-2 pb-0",
    ...
    >
      <div
        {...trackProps}
        className={cn("mr-video-editor-theme",
          "relative h-2 w-full touch-none rounded-full bg-[var(--muted)]",
          size === "sm" && "h-1",
          TOUCH_GESTURE_CLASS,
      ...
      >
        <motion.div
          className="absolute inset-y-0 left-0 rounded-full bg-[var(--foreground)]/40"
          style={{ width: left }}
        />
        ...
        <motion.div
          className={cn("mr-video-editor-theme",
            "absolute top-1/2 size-5 rounded-full border-2 border-[var(--foreground)] bg-[var(--background)]",
            size === "sm" && "size-3",
          )}
        ...
        >
          <AnimatePresence>
            {dragging ? (
              <motion.div
                initial={
                  reduce ? { opacity: 0 } : { opacity: 0, scale: 0.4, y: 10 }
                }
                animate={
                ...
                className="relative whitespace-nowrap rounded-[var(--ui-radius-md)] border border-[var(--border)] bg-[var(--popover)] px-2.5 py-1 text-sm font-medium tabular-nums text-[color:var(--popover-foreground)] shadow-[var(--ui-shadow-popover)]"
              >
                {readout}
                <span className="absolute -bottom-1 left-1/2 size-2.5 -translate-x-1/2 rotate-45 rounded-[var(--ui-radius-xs)] bg-[var(--popover)]" />
              </motion.div>
            ) : null}
          </AnimatePresence>
        </motion.div>
        ...
        <button
          ...
          {...sliderProps}
          className={cn("mr-video-editor-theme",
            "absolute -inset-y-5 inset-x-0 touch-none rounded-[var(--ui-radius-sm)] outline-none focus-visible:ring-2 focus-visible:ring-[var(--ring)]",
            size === "sm" && "-inset-y-3.5",
          )}
        />
      </div>
