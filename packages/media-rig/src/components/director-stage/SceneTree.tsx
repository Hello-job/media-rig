    ...
    <div className="director-stage__tree">
      {groups.map((group) => {
        ...
        return <div key={group.title}>
          <Button variant="ghost" type="button" className="director-stage__tree-title" aria-expanded={open} onClick={() => setCollapsed((previous) => ({ ...previous, [group.title]: open }))}>
          ...
          {items.map(({ kind, item }) => {
            ...
            return (
              ...
              >
                {renaming === item.id ? <input autoFocus aria-label={`重命名 ${item.label}`} value={draftName} onFocus={(event) => event.target.select()} onChange={(event) => setDraftName(event.target.value)} onBlur={() => { if (draftName.trim()) onRename(itemSelection, draftName.trim()); setRenaming(null); }} onKeyDown={(event) => { if (event.key === "Enter") event.currentTarget.blur(); if (event.key === "Escape") { event.stopPropagation(); setRenaming(null); } }} /> : <Button variant="ghost" type="button" className="director-stage__tree-select" onClick={() => onSelect(itemSelection)} style={{ opacity: item.visible ? 1 : 0.4 }}>
                ...
                <div className="director-stage__tree-actions">
                  ...
                  <Button variant="ghost" type="button" title="重命名" aria-label={`重命名 ${item.label}`} onClick={() => { setDraftName(item.label); setRenaming(item.id); }}><Pencil size={14} /></Button>
                  <Button variant="ghost"
                    type="button"
