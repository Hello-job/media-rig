      ...
      <div className="image-editor__layer-list">
        {[...document.objects].reverse().map((object) => (
          <div key={object.id} className={selectedIds.includes(object.id) ? "is-selected" : undefined}>
            <Button variant="ghost" size="icon" type="button" className="image-editor__layer-name" onClick={() => actions.selectObject(object.id)}>
              <span>{object.name}</span>
              <small>{object.type}</small>
            </Button>
            <Button variant="ghost" size="icon" type="button" aria-label={object.visible ? `隐藏 ${object.name}` : `显示 ${object.name}`} onClick={() => {
              actions.selectObject(object.id);
              actions.toggleVisibility();
            }}>
              {object.visible ? <Eye size={16} /> : <EyeOff size={16} />}
            </Button>
            <Button variant="ghost" size="icon" type="button" aria-label={object.locked ? `解锁 ${object.name}` : `锁定 ${object.name}`} onClick={() => {
              actions.selectObject(object.id);
              actions.toggleLock();
            }}>
